...
class mutate_mxdatetime(XMLP_Mutator):
    ...
    def unmutate(self, mobj):
        ...
        fmt = r'YMD\s*=\s*([0-9]+)\s*/\s*([0-9]+)\s*/\s*([0-9]+)\s*,\s*'
        fmt += r'HMS\s*=\s*([0-9]+)\s*:\s*([0-9]+)\s*:\s*([0-9\.]+)'
        m = re.match(fmt, obj)
...
def olddata_to_newdata(data, extra, paranoia):
    ...
    if isinstance_any(o, (int, float, complex)) and isinstance(data, (str, unicode)):
        data = aton(data)
